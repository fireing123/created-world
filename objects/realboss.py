...
class RealBoss(GameObject):
    def __init__(self, name, layer, tag, visible, position, rotation, parent_name):
        ...
        self.head = ImageObject(self, path="./resource/boss/real/head.png")
        self.components.append(self.head)

        self.body = BossBody(name)
        self.body.init_instantiate()

        self.mini_arm = BossMiniArms(self.body, "./resource/boss/real/punch.png", (0.5, 1))
        self.childrens.append(self.mini_arm)

        self.arm = BossArms(name, self.body, "./resource/boss/real/punch.png", (1, 1))
        self.childrens.append(self.arm)

        self.wall = BossWall([0, 10000], name)
        self.childrens.append(self.wall)

        self.follow_arrow = FollowArrow(self.body, "./resource/boss/real/punch.png", (1, 0.5))
        self.childrens.append(self.follow_arrow)
        
        self.rotating_arm = RotatingArm(self.body, "./resource/boss/real/punch.png", (1, 1))
        self.childrens.append(self.rotating_arm)

        self.page_wait = TimerTask(10000)
    ...
    def arrow_attack(self):
        if self.arrow_wait.run_periodic_task():
            ...
            arrow = BossArrow([2400, 1400], vector, self.body, "./resource/boss/real/punch.png", (0.25, 0.5))
            arrow.instantiate()
    ...
    def start_boss(self):
        ...
        self.location.position = [2400, 1300]
    ...
    def random_page(self):
        self.page(random.choice(range(4)))
...
class BossBody(GameObject):
    def __init__(self, parent_name):
        super().__init__(parent_name+"_body", 2, "living", True, [0, -200], 0, parent_name)
        self.image = ImageObject(self, value=Manger.surface_sheet["boss"].images[0])
        ...
        self.sound = SoundSource(self, "./resource/sound/boss/hit.wav", 0, lambda dist: 0.3)
    ...
    def reset(self):
        self.heath = 1000
        boss: 'RealBoss' = self.location.parent.object
        ...
        if self.invincibility.run_periodic_task():
            ...
            if self.heath < 0:
                ...
                def end_world():
                    ...
                    Game.stop("real")
                self.world_1.talk_start("real_boss_die", end_world)
