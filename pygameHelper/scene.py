...
import pygame
import pygameHelper.util as _util
from pygameHelper.objects import *
from pygameHelper.objects.components.physics import physics_objects

from pygameHelper.manger import Manger
from pygameHelper.sheet import TileSheet, SurfaceSheet

from typing import List, TYPE_CHECKING, Sequence

if TYPE_CHECKING:
    from pygameHelper.objects.gameobject import GameObject
    from pygameHelper.objects.camera import CameraObject
...
class Scene:
    ...
    def load(self, path: str):
        ...
        gameobject_len = sum([len(i) for i in list(json['objs'].values())])
        ...
        for name in objs.keys():
            for json_object in objs[name]:
                ...
                try:
                    ...
                    prefab_class: 'GameObject' = Manger.classes.get(name, None)
                    if prefab_class == None:
                        prefab_class = globals().get(name, None)
                        if prefab_class == None:
